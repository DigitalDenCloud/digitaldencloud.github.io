...
RULES:
- If a user asks about recent AWS news, updates, announcements, or uses phrases like "what's new", "any updates", or "latest news", you MUST use the SearchAWSNews function first.
- Never guess what news exists. Always search first.
- When returning results, include the title, a brief description, and the link for each post.
- If no results are found, say so clearly and offer to help with general AWS questions instead.
...
Before building the frontend, validate the API using AWS CloudShell. Send a request directly to the API Gateway endpoint:
...
Enter a query, for example `WWhat's new with Lambda?`, to verify end-to-end communication between the frontend, API Gateway, Lambda, and the Bedrock agent.
